...
export function MapScreen() {
  ...
  const worldLevels = useMemo(() => LEVELS.filter(level => level.world === currentLevel.world), [currentLevel.world]);
  const worldStartId = worldLevels[0]?.id ?? 1;
  ...
  useEffect(() => {
    ...
    const index = Math.max(0, store.currentLevelId - worldStartId);
  ...
  }, [height, store.currentLevelId, worldStartId]);
      ...
      <ImageBackground source={mapBackground} style={styles.bg} resizeMode="cover">
        <SafeAreaView style={styles.safe} edges={['top']}>
          <View style={styles.header}>
            ...
            <Text style={styles.dragHint}>DRAG THE MAP TO EXPLORE</Text>
          </View>
          <ScrollView ref={scrollRef} style={styles.scroller} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} bounces>
            <View style={styles.mapCanvas}>
              {worldLevels.map((level, index) => {
                ...
                const left = `${X_PATH[index] * 100}%` as const;
                ...
                return (
                  <View key={level.id} style={[styles.nodeSlot, { left, top }]}>
                    {index < worldLevels.length - 1 ? <View style={[styles.trail, { transform: [{ rotate: X_PATH[index + 1] > X_PATH[index] ? '-20deg' : '20deg' }] }]} /> : null}
                    <Pressable
              ...
              <View style={styles.gate}>
                <Text style={styles.gateTitle}>{nextWorldNameForLevel(currentLevel.id)}</Text>
                <Text style={styles.gateText}>{stars}/{STAR_GATE} STARS TO UNLOCK</Text>
...
const styles = StyleSheet.create({
  ...
  gate: { position: 'absolute', top: 34, left: 28, right: 28, padding: 14, borderRadius: 18, borderWidth: 3, borderColor: '#d7ad4b', backgroundColor: 'rgba(55,34,14,0.92)' },
